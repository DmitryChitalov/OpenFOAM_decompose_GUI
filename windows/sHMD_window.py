...
QLineEdit, QVBoxLayout, QListWidgetItem, QTabWidget, QScrollArea, QFormLayout
...
class shmd_window_class(QWidget):
	def __init__(self, parent, par, prj_path_cur, mesh_name_txt_cur, pd_2_cur):
		...
		if castellatedMC_visible == True:
			...
			castellatedMC_1_null_lbl = QLabel()
		...
		if castellatedMC_visible == True and os.path.exists(castellatedMC_2_path):       
			...
			castellatedMC_2_null_lbl = QLabel()
		...
		bMD_frame = QFrame()
		bMD_frame.setFixedSize(670, 510)
		bMD_frame.setStyleSheet(open("./styles/properties_form_style.qss","r").read())
		bMD_frame.setFrameShape(QFrame.Panel)
		bMD_frame.setFrameShadow(QFrame.Sunken)
		bMD_frame.setLayout(bMD_grid)

		fvS_vbox = QVBoxLayout() 
	...
	def on_initial_btnSave_clicked(self):
		...
		if mT_v == '':
			...
			if int_lng == 'Russian':
				...
				msg = "Initial data successfully saved"
			msg_list.append(msg)
	...
	def on_geometry_1_btnSave_clicked(self):
		...
		if int_lng == 'Russian':
			...
			msg = "The geometry_1 data successfully saved"
		self.on_msg_correct(msg)
...
###---------------------Сохранение вкладки geometry_2-------------------------###		
	...
	def on_castellatedMC_1_btnSave_clicked(self):
		...
		if len(cMC_obj) == 5:
			...
			if int_lng == 'Russian':
				...
				msg = "The 'castellatedMC_1' data successfully saved"
	...
	def on_castellatedMC_2_btnSave_clicked(self):
		...
		if f_v == True and True in tri_distirbTri_list:
			...
			if int_lng == 'Russian':
				...
				msg = "The 'castellatedMC_2' data successfully saved"
	...
	def on_layers_btnSave_clicked(self):
		...
		for el_base in range(layers_prs_base_table.rowCount()):
			...
			if layer_edit.text() == '':
				if int_lng == 'Russian':
					...
					msg = "Specify the name of " + + str(k) + " layer " 
				msg_list.append(msg)
			else:
		...
		if len(layers_list) == layers_prs_base_table.rowCount():
			...
			if int_lng == 'Russian':
				...
				msg = "The 'layers' data successfully saved"	
	...
	def on_snapC_btnSave_clicked(self):
		...
		if int_lng == 'Russian':
			...
			msg = "The 'snapC' data successfully saved"	
	...
	def on_meshQC_btnSave_clicked(self):
		...
		if len(mQC_prs_list) == len(mQC_chcks_list):
			...
			if int_lng == 'Russian':
				...
				msg = "The 'meshQC' data successfully saved"	
	...
	def on_msg_correct(self, msg):	
		...
		parn.item = QListWidgetItem()
		parn.listWidget.addItem(parn.item)
		parn.listWidget.setItemWidget(parn.item, msg_lbl)
	...
	def on_btnSave_clicked(self):
		# добавляем в таблицу
		if 'patches' not in parn.con.tables():	
			query = QtSql.QSqlQuery()
			query.exec("CREATE TABLE patches(patch_name)")
			j = 0
			for el_m in pte_list:
				query.exec("INSERT INTO patches(patch_name) VALUES ('%s')" % (''))
				patch_name = pne_list[j].text()
				query.prepare("UPDATE patches SET patch_name=? WHERE patch_name=''")
				query.bindValue(0, patch_name)
				query.exec_()
				j = j + 1
				
		if not os.path.exists(prj_path + '/' + 'snappyHexMeshDict'):	
			el_system = 'snappyHexMeshDict'
			item_system = parn.treeview.model.item(1, 0)
		...
		parn.file_open.setEnabled(True)
