...
# ------------------------------------Второй блок формы--------------------------------------#
		...
		regexp = QtCore.QRegExp('[А-яА-Яa-zA-Z0-9]+')
	...
	def on_path_choose(self):
		...
		if mesh_mas != []:
			...
			if int_lng == 'Russian':
				...
				dialog = QMessageBox(QMessageBox.Critical,
					"Attention!", "It's not mesh file. Choose another file",
					buttons = QMessageBox.Ok)
			result = dialog.exec_()
	...
	def on_finished(self):
		...
		if proc.returncode == 0:
			...
			if int_lng == 'Russian':
				...
				par.item = QListWidgetItem("Mesh successfully generated", par.listWidget)
			...
			if int_lng == 'Russian':
				...
				par.item = QListWidgetItem("Mesh is not generated", par.listWidget)
			...
			if self.nf_radio.isChecked() == True:
				if self.mesh_name.text() == '':
					...
					if self.bm.isChecked() == True:
						...
						if int_lng == 'Russian':
							par.ffw_label.setText("Форма подготовки расчетной сетки: " + "<font color='peru'>" + 'blockMesh' + "</font>")
						elif int_lng == 'English':
							par.ffw_label.setText("Form of preparation of the " + "<font color='peru'>" + 'blockMesh' + "</font>" + "mesh")
						...
						if int_lng == 'Russian':
							par.ffw_label.setText("Форма подготовки расчетной сетки: " + "<font color='peru'>" + 'snappyHexMesh' + "</font>")
						elif int_lng == 'English':
							par.ffw_label.setText("Form of preparation of the " + "<font color='peru'>" + 'snappyHexMesh' + "</font>" + "mesh")	
				...
				if pd_2 == 'blockMesh':
					...
					dir_system_name = os.path.basename(full_dir + '/system/blockMeshDict')
					if dir_system_name:
						item_system = par.treeview.model.item(0, 0)
