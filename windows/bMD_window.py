...
from PyQt5.QtWidgets import QWidget, QFileDialog, QLineEdit, QLabel, \
    ...
    QFrame, QVBoxLayout, QFormLayout, QTabWidget, QScrollArea, QListWidgetItem
...
class bmd_window_class(QWidget):
	def __init__(self, parent, par, prj_path_cur, mesh_name_txt_cur, pd_2_cur):
		...
		pd_2 = pd_2_cur
		mesh_name_txt = mesh_name_txt_cur + '_' + pd_2
		...
		if mergepatchpairs_visible == True and os.path.exists(patches_2_path):
			...
			mergepatchpairs_null_lbl = QLabel()
	...
	def on_blocks_1_btnSave_clicked(self):
		...
		if len(versh_list_main) == len(v_1_obsh_list) and len(yach_list_main) == len(v_2_obsh_list) and len(srya_list) == len(vrs_edit_list):
			...
			if int_lng == 'Russian':
				...
				msg = "The blocks_1 data successfully saved"
			self.on_msg_correct(msg)
	...
	def on_btnSave_clicked(self):
		...
		if not os.path.exists(prj_path + '/' + 'blockMeshDict'):	
			el_system = 'blockMeshDict'
			item_system = parn.treeview.model.item(0, 0)
